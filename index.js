import initColorPicker from "./shared/initColorPicker.js";
...
const { UI } = PDFAnnotate.default;
const documentId = 'https://arxiv.org/pdf/1708.08021.pdf';
let PAGE_HEIGHT;
let RENDER_OPTIONS = {
...
PDFAnnotate.default.setStoreAdapter(new PDFAnnotate.default.LocalStoreAdapter());
...
(function() {
  ...
  initColorPicker(document.querySelector('.hotspot-color'), hotspotColor, function(value) {
    ...
    // update annotation
    PDFAnnotate.default.getStoreAdapter().getAnnotation(documentId, annotationId).then((annotation) => {
      annotation.color = hotspotColor;
      PDFAnnotate.default.getStoreAdapter().editAnnotation(documentId, annotationId, annotation);
    });
  });
...
(function(window, document) {
  ...
  function handleAnnotationClick(target) {
    if (supportsComments(target)) {
      ...
      PDFAnnotate.default.getStoreAdapter().getComments(documentId, annotationId).then((comments) => {
        commentList.innerHTML = '';
        commentForm.style.display = '';
        commentText.focus();

        commentForm.onsubmit = function() {
          PDFAnnotate.default.getStoreAdapter().addComment(documentId, annotationId, commentText.value.trim())
            .then(insertComment)
            .then(() => {
              commentText.value = '';
              commentText.focus();
